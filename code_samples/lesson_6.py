...
len_set = len(set(proverbs))
number_proverbs = input('Сколько вы хотите получить пословиц? ')
if number_proverbs.isdigit():
    proverbs_count = int(number_proverbs)
    if proverbs_count > len_set:
        print(f"Ваш запрос {proverbs_count} превышает допустимое количество пословиц\n"
              f"Будет выведено {len_set} пословиц")
        proverbs_count = len_set
else:
    raise ValueError('Вы ввели не число')

for item in range(proverbs_count):
    # from random import choice
    # random_proverds = choice(proverbs)
    random_proverds = proverbs.pop()
    # random_proverds.replace('Ум', variants.pop())
    # print(random_proverds)
    if random_proverds.startswith('Ум'):
        random_sym = random_proverds.replace('Ум', variants.pop())
        print(random_sym.capitalize())
